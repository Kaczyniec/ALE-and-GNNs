    ...
    for no_exp in range(5):
        for n_layers in [2,3,4,5]:
            ...
            for _ in range(5):
                ...
                pd.DataFrame({'k': 2**n, 'max_bin_size': 2**max_bin_size, 'ALE exact': ale_exact, 'time_exact': t_exact, 'ALE approximate': ale_approximate, 'time_approximate': t_approximate, 'n_layers': n_layers, 'hidden_size': hidden_channels, 'no_exp': no_exp}
                            ).to_csv(os.path.join("data", 'citations', f"ALE_depth_GAT.csv"), mode='a', header=False)
            task.close()
            del model
