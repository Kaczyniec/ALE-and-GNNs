        ...
        for bin_idx in range(num_bins):
            ...
            for idx in tqdm(bin_data_idx_subset):
                ...
                subset, edge_index, mapping, edge_mask = k_hop_subgraph(
                    list(unique) + [idx], 4, data.edge_index, relabel_nodes=True
                )

                edge_label_index = torch.cat(
                    (
                        torch.Tensor(mapping[:-1]).int().unsqueeze(-1),
        ...
        for bin_idx in range(num_bins):
            ...
            edge_label_index = torch.cat(
                (
                    ...
                    .int()
                    .repeat_interleave(unique.shape[0])
                    .unsqueeze(-1),
                    mapping[len(bin_data_idx_subset) :]
                    .repeat(bin_data_idx_subset.shape)
                    .int()
                    .unsqueeze(-1),
                ),
                dim=1,
