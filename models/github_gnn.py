...
if __name__ == '__main__':
    ...
    node_features_path = 'data/git_web_ml/musae_git_features.json'
    data_path = 'data/git_web_ml/'
    model_path = f"models/git_web_ml/"+CONFIGS['model_type']+',n_layers'+str(CONFIGS['n_layers'])+',hidden_size'+str(CONFIGS['hidden_channels'])
    ...
    node_features = pd.DataFrame.from_dict(data_dict, orient='index').fillna(-1)#[['views', 'mature', 'life_time', 'dead_account', 'affiliate']]
    train_loader, test_loader, train_data, test_data = graph_data(edges, node_features, 'data/git_web_ml/', CONFIGS['batch_size'])
    print(train_data.validate())
